...
NSString* string_to_nsstring(String str) 
{
    return [[NSString alloc] initWithUTF8String:str.utf8().get_data()];
}

String nsstring_to_string(NSString* str) 
{
    const char *s = [str UTF8String];
    return String::utf8(s != NULL ? s : "");
}

NSArray* array_to_nsarray(Array arr)
{
    ...
    for (int i = 0; i < arr.size(); ++i) {
        ...
        if (value != NULL) {
            ...
            WARN_PRINT("FCM::Uils Trying to add something unsupported to the array.");
        }
    }
    return result;
}

Array nsarray_to_array(NSArray* array)
{
    Array result;
...
NSDictionary* dictionary_to_nsdictionary(Dictionary dic)
{
...
Dictionary nsdictionary_to_dictionary(NSDictionary* dic)
{
    Dictionary result;
...
Variant nsobject_to_variant(NSObject *object)
{
    if ([object isKindOfClass:[NSString class]]) {
        ...
        WARN_PRINT("FCM::Uils NSDate unsupported, returning null Variant");
        return Variant();
    } else if ([object isKindOfClass:[NSNull class]] or object == nil) {
        return Variant();
    } else {
        WARN_PRINT("FCM::Uils Trying to convert unknown NSObject type to Variant");
        return Variant();
    }
}
...
NSObject* variant_to_nsobject(Variant v)
{
    switch (v.get_type()) 
    {
        ...
        case Variant::DICTIONARY :
            return dictionary_to_nsdictionary(v);
        case Variant::ARRAY :
            return array_to_nsarray(v);
    ...
    WARN_PRINT(String("FCM::Uils Could not add unsupported type to iCloud: '" + Variant::get_type_name(v.get_type()) + "'").utf8().get_data());
    return NULL;
}
